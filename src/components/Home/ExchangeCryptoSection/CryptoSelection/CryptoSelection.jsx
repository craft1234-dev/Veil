...
import { useCrypto, useCryptoDispatch } from "../context/CryptoContext.js";

export default function CryptoSelection() {
  const crypto = useCrypto();

  let sendCrypto = crypto.sendCrypto;
  let receiveCrypto = crypto.receiveCrypto;
  ...
  return (
    <div className="row position-relative z-2 gy-4 mb-3 mb-md-4">
      <button
        ...
        type="button"
        onClick={() => dispatch({ type: "swapCrypto" })}
      >
        <svg
            ...
            >
              <div className="d-flex flex-row justify-content-start align-items-center">
                ...
                <div className="d-flex flex-column justify-content-center align-items-start ms-3">
                  <p className="mb-1 h5">{sendCrypto.name}</p>
                  <p className="mb-1 opacity-50">{sendCrypto.shortName}</p>
                </div>
              </div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="d-flex flex-row justify-content-start align-items-center">
                <div className="c-exchange__icon">
                  <img
                    ...
                    data-nimg="1"
                    style={{ color: "transparent" }}
                    src={receiveCrypto.icon}
                  />
                </div>
                <div className="d-flex flex-column justify-content-center align-items-start ms-3">
                  <p className="mb-1 h5">{receiveCrypto.name}</p>
                  <p className="mb-1 opacity-50">{receiveCrypto.shortName}</p>
                </div>
              </div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
